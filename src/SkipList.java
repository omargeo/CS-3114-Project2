...
public class SkipList<K extends Comparable<K>, V> {
     ...
     */
    public ArrayList<SkipNode<K, V>> search(K name) {
        ArrayList<SkipNode<K, V>> array = new ArrayList<SkipNode<K, V>>();
        SkipNode<K, V> x = head; // Dummy header node
//        for (int i = level; i >= 0; i--) { // For each level...
        while ((x.getForward()[0] != null)) { // go forward
            if ((x.getForward()[0].getKey().
                    compareTo(name) == 0)) {
                array.add(x.getForward()[0]);
            }
            x = x.getForward()[0]; // Go one last step
        }
//        }
